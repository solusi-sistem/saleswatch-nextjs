      ...
      <main className="flex-grow">
        ...
        <section className="py-16 px-6 md:px-12 bg-[#f2f7ff]">
          <div className="container mx-auto max-w-4xl text-center">
            <h1 className="text-4xl md:text-5xl font-bold mb-6 animate__animated animate__fadeInUp">{language === 'id' ? sectionData.title_section?.id : sectionData.title_section?.en || ''}</h1>
            {sectionData.desc_section && <p className="text-xl text-gray-600 max-w-3xl mx-auto animate__animated animate__fadeInUp">{language === 'id' ? sectionData.desc_section.id : sectionData.desc_section.en}</p>}
          </div>
        </section>
          ...
          <div className="container mx-auto">
                  ...
                  >
                    <article className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 h-full animate__animated animate__fadeInUp">
                      ...
                      <div className="p-6">
                        ...
                        {post.category && <span className="inline-block px-3 py-1 text-xs font-medium bg-[#061551] text-white rounded-full mb-3">{language === 'id' ? post.category?.id : post.category?.id}</span>}

                        <h2 className="text-xl font-bold mt-2 mb-3 line-clamp-2 group-hover:text-[#061551] transition">{language === 'id' ? post.title?.id : post.title?.en}</h2>

                        <p className="text-gray-700 mb-4 line-clamp-3">{language === 'id' ? post.excerpt?.id : post.excerpt?.en}</p>
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center flex-wrap gap-2">
                {currentPage > 1 && (
                  <button onClick={() => handlePageChange(currentPage - 1)} className="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 transition text-sm">
                ...
                {getPageRange().map((pageNum) => (
                  <button
                    ...
                    className={`w-10 h-10 flex items-center justify-center rounded-lg text-sm transition ${currentPage === pageNum ? 'bg-[#061551] text-white' : 'bg-white border border-gray-300 text-gray-700 hover:bg-gray-50'}`}
                  >
                    {pageNum}
                  </button>
                ))}
