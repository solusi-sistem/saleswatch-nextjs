...
export default function HeroUtama({ id, data }: SectionProps) {
  const pathname = usePathname();
  const locale: LangKey = pathname.startsWith("/id") ? "id" : "";

  const [content, setContent] = useState<HeroUtamaContent | null>(
    data?.hero_content || null,
  );
  const [loading, setLoading] = useState(!data?.hero_content);
  ...
  useEffect(() => {
    if (data?.hero_content) {
      setContent(data.hero_content);
      setLoading(false);
      return;
    }

    // Fallback fetching only if data isnt provided
    async function fetchContent() {
      ...
      try {
        ...
        console.error("HeroUtama fetch error:", error);
      } finally {
        setLoading(false);
      }
    }
  ...
  const titleText = content.title_lines
    ? locale === "id"
      ? content.title_lines.text_id
      : content.title_lines.text_en
    : "";

  const descriptionText = content.description_lines
    ? locale === "id"
      ? content.description_lines.text_id
      : content.description_lines.text_en
    : "";

  const ctaButtonText = content.cta_button
    ? locale === "id"
      ? content.cta_button.text_id
      : content.cta_button.text_en
    : "";

  const speechBubbleLines = content.speech_bubble
    ? locale === "id"
      ? content.speech_bubble.text_lines_id
        ...
        <div className="relative w-full overflow-hidden rounded-4xl">
          ...
          <div className="relative z-10 flex flex-col items-center justify-center px-4 py-12 md:py-20 mt-5 md:mt-0 text-center text-[#DFE1E4]">
            <div className="max-w-3xl">
              {titleText && (
                <h1 className="md:mb-4 text-3xl font-bold tracking-tight sm:text-4xl md:text-5xl lg:text-5xl animate__animated animate__fadeIn">
                  {titleText}
                </h1>
              )}
            </div>
            ...
            {descriptionText && (
              <div className="mt-4 md:mt-6 mb-6 md:mb-8">
                <p className="max-w-2xl text-md text-gray-200 text-md md:text-xl animate__animated animate__fadeIn">
                  {descriptionText}
                </p>
              </div>
            )}
            ...
            {images.length > 0 && (
              <>
                <div
                  className={`relative w-full max-w-[1200px] h-[200px] md:h-[250px] md:h-[450px] overflow-hidden transition-all duration-1000 ease-out ${isVisible ? "translate-y-0 opacity-100" : "translate-y-20 opacity-0"}`}
                >
                  <div
                  ...
                  >
                    {images.map((src, index) => (
                      <div
                        key={index}
                        className="relative min-w-full h-full flex items-center justify-center px-4"
                      >
                        <div
                          className="relative w-full max-w-[800px] h-[450px] mx-auto overflow-hidden"
                          style={{ clipPath: "inset(45px 5px 0 0)" }}
                        >
                          <Image
                            src={src}
                            alt={`Slide ${index + 1}`}
