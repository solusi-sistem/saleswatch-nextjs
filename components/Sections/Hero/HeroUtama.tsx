...
export default function HeroUtama({ id }: SectionProps) {
    ...
    const titleText = content.title_lines
        ? (locale === 'id' ? content.title_lines.text_id : content.title_lines.text_en)
        : '';

    const descriptionText = content.description_lines
        ? (locale === 'id' ? content.description_lines.text_id : content.description_lines.text_en)
        : '';

    const ctaButtonText = content.cta_button
        ? (locale === 'id' ? content.cta_button.text_id : content.cta_button.text_en)
        : '';

    const speechBubbleLines = content.speech_bubble
        ? (locale === 'id' ? content.speech_bubble.text_lines_id : content.speech_bubble.text_lines_en)
        : '';
                        ...
                        <div className="grid grid-cols-2 md:flex md:flex-wrap md:justify-center md:items-start gap-6 md:gap-8 lg:gap-16 xl:gap-20 max-w-6xl mx-auto w-full">
                            {statistics.map((stat, index) => {
                                ...
                                return (
                                    <div
                                        key={index}
                                        className="flex flex-col items-center text-center md:min-w-[120px] lg:min-w-[150px]"
