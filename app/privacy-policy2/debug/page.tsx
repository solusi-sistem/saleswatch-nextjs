...
import { getPrivacyPolicyData } from '@/lib/sanity.privacyPolicy';
import type { PrivacyPolicySection } from '@/types/privacyPolicy';

export default function PrivacyPolicyDebugPage() {
  const { data: contextData, loading: contextLoading, error: contextError, refetch } = usePrivacyPolicy();
  const [directData, setDirectData] = useState<PrivacyPolicySection | null>(null);
  ...
  const contextItemsCount = contextData?.privacy_policy_content?.items?.length || 0;
  const directItemsCount = directData?.privacy_policy_content?.items?.length || 0;
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-gray-900 text-white p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl p-8 mb-6 shadow-2xl">
          <h1 className="text-4xl font-bold mb-2">🔍 Privacy Policy Debug</h1>
          <p className="text-blue-100">Comprehensive debugging information for Privacy Policy data</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div className={`rounded-xl p-6 ${contextItemsCount > 0 ? 'bg-green-600' : 'bg-yellow-600'}`}>
            ...
            <div className="text-sm opacity-90">Policy Items</div>
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 mb-6 border border-white/20">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center pb-2 border-b border-white/20">
              ...
              <span className={`font-semibold ${contextData?.privacy_policy_content ? 'text-green-400' : 'text-red-400'}`}>
                {contextData?.privacy_policy_content ? '✅ Yes' : '❌ No'}
              </span>
            </div>
            <div className="flex justify-between items-center pb-2 border-b border-white/20">
        ...
        {/* Items List */}
        {contextData?.privacy_policy_content?.items && contextData.privacy_policy_content.items.length > 0 && (
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 mb-6 border border-white/20">
            <h2 className="text-2xl font-bold mb-4">📝 Privacy Policy Items</h2>
            <div className="space-y-3">
              {contextData.privacy_policy_content.items.map((item, index) => (
          ...
          <div className="space-y-4 text-sm">
            <div>
              ...
              <ul className="list-disc list-inside space-y-1 text-white/80 ml-4">
                <li>Check if section exists in Sanity with type_section = "privacyPolicySection"</li>
                <li>Ensure published_at = true for the section</li>
                <li>Verify privacy_policy_content field has items array</li>
                <li>Check if items are referenced (not embedded)</li>
                <li>Ensure list_privacy_policy documents have published_at = true</li>
              </ul>
            </div>
            <div>
...
{`*[_type == "section" && type_section == "privacyPolicySection"] {
  _id,
  name_section,
  published_at,
