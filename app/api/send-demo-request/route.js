  ...
  try {
    ...
    if (!smtpHost || !smtpUser || !smtpPass || !mailTo) {
      ...
      return NextResponse.json(
        {
          error:
            "Konfigurasi email (SMTP) tidak lengkap di Environment Variables (.env).",
        },
        { status: 500 },
      );
    }

    // Jika port 465 biasanya secure: true (SSL), jika 587 secure: false (TLS)
    ...
    try {
      ...
      return NextResponse.json(
        {
          error:
            "Koneksi ke server email gagal. Periksa username/password SMTP di .env.",
    ...
    // Bersihkan nomor whatsapp untuk link
    const cleanWhatsapp = whatsapp.replace(/[^0-9]/g, "");

    const mailOptions = {
      from: `"Saleswatch" <${smtpUser}>`,
      to: mailTo,
      ...
      text: `
        ...
        Hubungi via WhatsApp: https://wa.me/${cleanWhatsapp}
