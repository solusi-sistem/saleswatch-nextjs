...
const customPortableTextComponents = {
  types: {
    image: ({ value }: any) => {
      ...
      return (
        <div className="my-6">
          <Image src={value.asset.url} alt={value.alt || 'Blog image'} width={800} height={450} className="rounded-lg w-full" />
            ...
            <div className="lg:w-2/3">
              ...
              {post.image && (
                <div className="mb-8 rounded-lg overflow-hidden animate__animated animate__fadeInUp">
                  <Image src={post.image.asset.url} alt={post.image.alt?.[language] || post.title[language]} width={800} height={450} className="w-full object-cover" priority />
                </div>
              )}
              ...
              {/* Title */}
              <h1 className="text-3xl md:text-4xl font-bold mb-6 animate__animated animate__fadeInUp">{post.title[language]}</h1>

              {/* Content using PortableText */}
              <div className="prose prose-lg max-w-none mb-10 animate__animated animate__fadeInUp">{post.content?.[language] && <PortableText value={post.content[language]} components={customPortableTextComponents} />}</div>
            ...
            <div className="lg:w-1/3 space-y-8">
              ...
              {recentPosts && recentPosts.length > 0 && (
                <div className="bg-gray-50 p-6 rounded-lg">
                  <h3 className="font-bold text-lg mb-4 animate__animated animate__fadeInUp">Kategori</h3>
                  <ul className="space-y-2">
                    {recentPosts.map((data, index) => (
                      <li key={index}>
                        <Link href={`/id/blog?category=${data?.category?.id}` || ''} className="hover:text-blue-600 transition animate__animated animate__fadeInUp">
                          {data?.category?.id}
                        </Link>
                      </li>
                    ))}
              ...
              {recentPosts && recentPosts.length > 0 && (
                <div className="bg-gray-50 p-6 rounded-lg">
                  <h3 className="font-bold text-lg mb-4 animate__animated animate__fadeInUp">Artikel Terbaru</h3>
                  <div className="space-y-4">
                    {recentPosts.slice(0, 5).map((item) => (
                      <Link key={item._id} href={`/blog/${item.slug.current}`} className="flex gap-3 hover:opacity-75 transition animate__animated animate__fadeInUp">
                        {item.image && (
                          <div className="w-16 h-16 rounded overflow-hidden flex-shrink-0">
                            <Image src={item.image.asset.url} alt={item.image.alt?.[language] || item.title[language]} width={64} height={64} className="object-cover w-full h-full" />
                          </div>
                        )}
                        <div>
              ...
              {post.tags && post.tags.length > 0 && (
                <div className="bg-gray-50 p-6 rounded-lg">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {post.tags.map((tag, index) => (
                      <span key={index} className="bg-white px-3 py-1 rounded text-sm border hover:border-[#061551] transition cursor-pointer animate__animated animate__fadeInUp">
                        {tag}
                      </span>
                    ))}
                  </div>
