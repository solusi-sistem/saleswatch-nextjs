              ...
              <tr>
                ...
                {plans.map((plan) => (
                  <th key={plan.name} className={`py-5 px-4 min-w-[180px] text-center ${plan.bg} ${plan.border} relative`}>
                    ...
                    <a href="/demo" className="inline-block w-full py-2 bg-[#6587A8] hover:bg-[#CFE3C0] hover:text-[#6587A8] text-white font-medium rounded-md transition text-sm">
                      Contact Sales
                    </a>
                  </th>
                ))}
                ...
                <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  {row.values.map((value, planIdx) => (
                    <td key={planIdx} className={`py-4 px-4 align-top text-sm ${plans[planIdx].bg} ${plans[planIdx].border} text-center`}>
                      {row.isFlex ? renderFeatureValue(value, true) : renderFeatureValue(value)}
                    </td>
                  ))}
                </tr>
              ))}
          ...
          {plans.map((plan, planIdx) => (
            <div key={plan.name} className={`rounded-xl border ${plan.border} ${plan.bg} overflow-hidden shadow-sm`}>
              ...
              <div className="p-5 text-center border-b border-gray-200">
                ...
                <a href="/demo" className="mt-3 inline-block w-full py-2 bg-[#6587A8] hover:bg-[#CFE3C0] hover:text-[#6587A8] text-white font-medium rounded-md transition text-sm">
                  Contact Sales
                </a>
              </div>
              ...
              <div className="p-5 space-y-4">
                {rows.map((row) => (
                  <div key={row.feature} className="space-y-1">
                    ...
                    <div className="text-sm text-gray-700">{row.isFlex ? renderFeatureValue(row.values[planIdx], true) : renderFeatureValue(row.values[planIdx])}</div>
                  </div>
                ))}
              </div>
            </div>
